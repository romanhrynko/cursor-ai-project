...
export function Sidebar() {
  ...
  return (
    <aside
      className={`relative flex flex-col h-screen bg-white dark:bg-[#f8fafc] border-r border-gray-100 dark:border-gray-800 shadow-sm rounded-none p-6 transition-all duration-300 ${open ? 'w-64' : 'w-16 px-2'}`}
    >
      ...
      {/* Logo/Title */}
      <div
        className={`mb-10 mt-2 flex items-center gap-2 select-none transition-all duration-300 ${open ? 'opacity-100' : 'opacity-0 pointer-events-none h-0 mb-0'}`}
        style={{ minHeight: open ? 'auto' : 0 }}
      >
        <span className="text-2xl font-extrabold tracking-tight text-gray-900">
          Tavily <span className="font-light">AI</span>
        </span>
      </div>
      {/* Navigation */}
          ...
          {navItems.map((item) => {
            ...
            return (
              <li key={item.name}>
                ...
                >
                  <div
                    className={`flex items-center gap-3 py-2 w-full ${open ? 'px-3' : 'justify-center'}`}
                  >
                    <item.icon
      ...
      {/* User info at bottom */}
      <div
        className={`mt-auto flex items-center gap-3 pt-8 transition-all duration-300 ${open ? '' : 'justify-center'}`}
      >
