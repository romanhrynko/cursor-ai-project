...
export function PlanCard({ plan, usage, limit, visible }: PlanCardProps) {
  return (
    <div
      className={`rounded-2xl shadow-lg mb-8 p-6 bg-gradient-to-r from-purple-500 via-pink-500 to-indigo-500 relative overflow-hidden transform transition-all duration-700 ${visible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-8 scale-95'}`}
    >
        ...
        <div>
          ...
          <div className="flex items-center gap-2 text-white/80 text-sm">
            ...
            <span className="relative group cursor-pointer">
              ...
              <span className="absolute left-1/2 -translate-x-1/2 mt-2 w-40 bg-black text-white text-xs rounded p-2 opacity-0 group-hover:opacity-100 transition pointer-events-none z-10">
